...
with open('../versions.alt/versions.orig/v41/statementResult.csv', 'r') as csvfile:
	...
	# 	rank = numpy.argsort(-score[i])
		...
		rows = zip(tarantula, ochiai, jaccard, hamman, fault)
		
		with open('score/41/score.csv', 'w') as f:
			writer = csv.writer(f)
			for row in rows:
				writer.writerow(row)
	...
	# # print(worstRank)
	# print(prediction)
	# with open('score/2/rankComparison.csv', 'w') as f:
	# 	writer = csv.writer(f)
	# 	# for row in header:
