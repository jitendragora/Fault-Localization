...
with open("RankingMetricsScore_tcasMutantLine63 (copy).csv", 'r', newline='') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
    	data = list(row)
    	#print(type(x) for x in data[:-1])
    	X.append([i for i in data[:-1]])
    	# np.append(y, data[-1])
...
#temp = 'score/' + str(i) + '/score.csv'
with open("RankingMetricsScore_tcasMutantLine72 (copy).csv", 'r', newline='') as csvfile:
	reader = csv.reader(csvfile)
	for row in reader:
		data = list(row)
